...
void Commander500ms(void)
{
    if (AppStartFlag > 0)
    {
        ...
        {
            CommanderSendNoise(Noise1Value, Noise2Value, 10000);
            CommanderNoiseConnect(GetNoise1Detect() , GetNoise2Detect() , 111111);
            CommanderSendNoiseValue(Noise1DB, NoiseDB, SmoothingFilterValue);
            CommanderSendNoiseValueOffset(AudioVolume, AudioNdOffsetAim, AudioNdedVolume);
        }
...
void CommanderNoiseConnect(int db1, int db2, int db)

{
    memset(OutStr, 0, sizeof(OutStr));
    sprintf(OutStr, "Noise1 Connect: %d	\nNoise2 Connect: %d  \nNoise dB: %d   \n\n", db1, db2, db);
    CommanderSendString(OutStr);
}

void CommanderSendNoiseValue(int db1, int db2, int db)
{
    memset(OutStr, 0, sizeof(OutStr));
    sprintf(OutStr, "NoiseV1 dB: %d  \nNoiseV2 dB: %d  \nNoiseOffer dB: %d   \n\n", db1, db2, db);
    CommanderSendString(OutStr);
}

void CommanderSendNoiseValueOffset(int db1, int db2, int db)
{
    memset(OutStr, 0, sizeof(OutStr));
    sprintf(OutStr, "Volume dB: %d  \nNoiseOffset dB: %d  \nNoiseVolume dB: %d   \n\n", db1, db2, db);
    CommanderSendString(OutStr);
}
...
{
    memset(OutStr, 0, sizeof(OutStr));
    sprintf(OutStr, "Sensor1 Noise (temp): %d  \n", noise1);
    CommanderSendString(OutStr);
    sprintf(OutStr, "Sensor2 Noise (temp): %d  \n", noise2);
    CommanderSendString(OutStr);
}
