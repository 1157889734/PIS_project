...
void AudioInit(void)
{
    ...
    soundcard_init();
    Delay(1000);
    soundcard_set_input_vol(8); //26 is a suitable value for sampling.
    Delay(1000);
    I2s2_Init();
    I2s3_Init();
    DmaPlay_Init();
...
int AudioSetVolume(int inda)
{
	...
	if (NoiseMode == 1)
	{
	    AudioNdedVolume = AudioVolume - AudioNdOffsetAim;
    ...
    soundcard_set_output_vol((u8)(AudioNdedVolume));
    //EmeVolumeSet();
    StorageInfoSave();
    return 0;
}
...
//  int ndvoffset;
    ...
    if ((AudioPlayInfo.EnableFlag < 1) || (AudioPlayInfo.Type == 9))
    {
        AudioNdOffsetAim = ndv - 60;
    }
    else
    {
#if 0
        AudioNdOffsetAim = ndv - 67;
		
        ndvoffset = AudioNdOffsetAim - AudioNdOffsetAim1;
        
        if (AudioNdOffsetAim > LIMITMAX)
        {
            AudioNdOffsetAim  --;
        }
        ...
        if (fabs(ndvoffset) < 2)
        {
            AudioNdOffsetAim = AudioNdOffsetAim1;
        }
        else if (fabs(ndvoffset) >= 2 )
        {
            AudioNdOffsetAim = AudioNdOffsetAim;
        }
#endif
    }
    
//  AudioNdOffsetAim1 = AudioNdOffsetAim;
    AudioNdFlag = 16;
    return 0;
}
